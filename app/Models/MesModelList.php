...
class MesModelList extends Authenticatable
{
    ...
    public static function getBuyDelayAjax($searchtype)
    {
        if ($searchtype == 'MaterialDeliveryDate') {
            $value = DB::select("SELECT * ,datediff(`dat_por`,now()) as date_gap FROM buy_delay where date_por_gap > 0 order by dat_buy,NUM_buy;");
        } else {
            $value = DB::select("SELECT * ,datediff(`dat_por`,now()) as date_gap FROM buy_delay where date_por_gap = 0 order by dat_buy,NUM_buy,dat_req;");
        }
        return $value;
    }
    ...
    public static function getRMAAnalysisAjax($type, $search, $rangS, $rangE)
    {
        if ($type == 'cod_cust') {
            $value = DB::select("SELECT *, DATEDIFF(`dat_acte`, DAT_ONCA) AS date_gap 
                                ...
                                AND `num_ser` BETWEEN '$rangS%' AND '$rangE%' 
                                ORDER BY `num_ser`");
        } elseif ($type == 'DAT_ACTB' || $type == 'num_mtrm' || $type == 'DD') {
            $value = DB::select("SELECT *, DATEDIFF(`dat_acte`, DAT_ONCA) AS date_gap 
                                FROM `rma_analysis` 
